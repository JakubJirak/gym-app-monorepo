...
export const getUserWorkouts = query({
	...
	async handler(ctx) {
		...
		const workouts = await ctx.db
			...
			.order("desc") // podle _creationTime v indexu (ponecháno, ale dál overridnuto řazením podle workoutDate)
			.collect();

		// Seřadíme podle workoutDate (formát "YYYY-MM-DD") — nejnovější nahoře
		workouts.sort((a, b) => {
			const ta = a?.workoutDate ? new Date(a.workoutDate).getTime() : Number.NEGATIVE_INFINITY;
			const tb = b?.workoutDate ? new Date(b.workoutDate).getTime() : Number.NEGATIVE_INFINITY;
			return tb - ta;
		});
...
export const deleteWorkout = mutation({
	...
	handler: async (ctx, args) => {
		...
		for (const w of workoutExercises) {
			const sets = await ctx.db
				.query("sets")
				.withIndex("by_workoutExerciseId", (q) => q.eq("workoutExerciseId", w._id))
				.collect();
