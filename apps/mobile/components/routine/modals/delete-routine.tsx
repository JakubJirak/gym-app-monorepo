...
export default function DeleteRoutineModal({ sheetName, routineId }: DeleteRoutineModalProps) {
	const name = sheetName ?? NAMES.sheets.deleteRoutine;
	const closeSheet = () => {
		...
		<TrueSheet
			...
			footer={() => (
				...
				>
					...
					<Text className="px-2 py-1 text-center font-bold text-lg text-text">Smazat rutinu</Text>
				</TouchableOpacity>
			)}
		...
		>
			<View className="px-4 pt-8 pb-18">
				<View className="mt-1 mb-4 flex-row items-center justify-center gap-2">
					...
					<Text className="font-bold text-text text-xl">rutinu</Text>
				</View>
